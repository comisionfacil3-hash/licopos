...
export default function CajaPage() {
  ...
  const loadCaja = async () => {
    ...
    try {
      ...
      if (cajaData) {
        await loadMovimientos(cajaData.id)
        await calcularResumen(cajaData)
      }
    ...
    <div className="p-4 pb-24 max-w-4xl mx-auto">
      ...
      {!cajaAbierta ? (
        ...
        <div className="space-y-4">
          ...
          {resumen && (
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              <h3 className="font-medium text-gray-900 mb-3">Desglose</h3>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">💵 Monto inicial efectivo</span>
                  <span className="text-gray-900">{formatCurrency(resumen.monto_inicial)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">📱 Monto inicial QR</span>
                  <span className="text-gray-900">{formatCurrency(resumen.monto_inicial_qr)}</span>
                </div>
                <div className="border-t border-gray-100 my-2"></div>
                <div className="flex justify-between text-emerald-600">
                  <span>+ Ventas efectivo</span>
                  <span>{formatCurrency(resumen.ventas_efectivo)}</span>
                </div>
                <div className="flex justify-between text-emerald-600">
                  <span>+ Ventas QR</span>
                  <span>{formatCurrency(resumen.ventas_qr)}</span>
                </div>
                {resumen.gastos_efectivo > 0 && (
                  <div className="flex justify-between text-red-600">
                    <span>- Gastos efectivo</span>
                    <span>{formatCurrency(resumen.gastos_efectivo)}</span>
                  </div>
                )}
                {resumen.gastos_qr > 0 && (
                  <div className="flex justify-between text-red-600">
                    <span>- Gastos QR</span>
                    <span>{formatCurrency(resumen.gastos_qr)}</span>
                  </div>
                )}
                {resumen.retiros > 0 && (
                  <div className="flex justify-between text-red-600">
                    <span>- Retiros</span>
                    <span>{formatCurrency(resumen.retiros)}</span>
                  </div>
                )}
                <div className="border-t border-gray-100 pt-2">
                  <div className="flex justify-between font-medium">
                    <span className="text-gray-900">💵 Total Efectivo</span>
                    <span className="text-gray-900">{formatCurrency(resumen.total_efectivo)}</span>
                  </div>
                  <div className="flex justify-between font-medium mt-1">
                    <span className="text-gray-900">📱 Total QR</span>
                    <span className="text-gray-900">{formatCurrency(resumen.total_qr)}</span>
                  </div>
                </div>
              </div>
            </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {mov.metodo_pago && (
                          <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-600">
                            {getMetodoPagoIcon(mov.metodo_pago)} {getMetodoPagoText(mov.metodo_pago)}
                          </span>
                        )}
                      </div>
            ...
            <div className="p-6 space-y-4">
              ...
              {resumen && (
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="text-xl font-bold text-gray-900">{formatCurrency(resumen.total_efectivo)}</p>
                </div>
              )}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
